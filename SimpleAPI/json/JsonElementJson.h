...
public:
    ElementJson() noexcept : m_values{}                     { m_type = ValueType::eArray; }
    ElementJson(const ElementJson& json) noexcept : m_values{}
                                                            { /*TODO*/ }
    ElementJson(const JPair& pair) noexcept : m_values{}    { /*TODO*/put(pair.first, pair.second); }
    //разнесено для решения конфликта, не изменять следующие два конструктора!
    ElementJson(const std::string& input_string) noexcept : m_values{}
                                                            { /*TODO*/parseJSON(input_string); }
    ElementJson(const std::string& input_string, const ConfigFormat config_format,
                const bool enable_comment = false)          { /*TODO*/ }
    __ONLY_ALLOWED_TYPES__(T)
    ElementJson(const std::string& key, const T& value) noexcept : m_values{}
                                                            { /*TODO*/put(key, value); }
    ElementJson(const JVector& vec) noexcept : m_values{}   { /*TODO*/ }
    ElementJson(const Element& element) noexcept : m_values{}
                                                            { /*TODO*/ }
    ...
    //TODO: put
    __ONLY_ALLOWED_TYPES__(T)
    ElementJson&    put(const std::string& key, const T& value, const bool rewrite = true) noexcept
    { return put(key, Element(value), rewrite); }
    ElementJson&    put(const std::string& key, const Element& element, const bool rewrite = true) noexcept;
    ElementJson&    put(const ElementJson& json, const bool rewrite = true) noexcept;

    __ONLY_ALLOWED_TYPES__(T)
    ElementJson&    add(const std::string& key, const T& value,
                        const bool rewrite = true) noexcept     { return put(key, value, rewrite); }
    ElementJson&    add(const ElementJson& json, const bool rewrite = true) noexcept
                                                                { return put(json, rewrite); }
    ElementJson&    append(const ElementJson& json, const bool rewrite = true) noexcept
                                                                { return put(json, rewrite); }
    ...
    ElementJson&            erase(const std::string& key) noexcept;
