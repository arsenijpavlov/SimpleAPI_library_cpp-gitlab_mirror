...
protected:
    ...
    struct KeysComments : KeysBase {
        std::string m_comment_str; //указатель для экономии памяти, всё равно объект статичный

        KeysComments(const std::string& comment)            noexcept : m_comment_str(comment)       {}
        KeysComments(const KeysComments& other)             noexcept;
        KeysComments(KeysComments&& other)                  noexcept;
        ~KeysComments()                                     noexcept override                       {}
