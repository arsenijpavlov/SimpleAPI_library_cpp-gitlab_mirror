                            ...
                            const bool map_sort_important) const noexcept
{
    ...
    // тип other может быть либо строкой, состоящей из одного символа, либо самостояльный символ
    switch(other.getType()){
    case ValueType::eChar:
        b2 = m_value == reinterpret_cast<const ElementChar&>(other).getValue();
        break;
    case ValueType::eString:
        b2 = m_value == !other.empty() && reinterpret_cast<const ElementString&>(other).getValue().at(0);
        break;
