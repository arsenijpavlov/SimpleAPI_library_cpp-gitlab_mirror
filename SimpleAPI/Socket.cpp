...
void UDPSocket::tick() {
    ...
    if(jRecv.contains("ping")) { //только для первого подключения
        std::string ipPortString = jRecv["ping"].getString();
        IpPort remoteIpPort;
        if(remoteIpPort.from_string(ipPortString))
            acknowledgeMap.insert(std::pair<IpPort, Json>(remoteIpPort, jRecv));
    }
